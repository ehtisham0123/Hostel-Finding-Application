...
router.get("/avatar/", verifyToken, async function (req, res, next) {
  try {
    var sql = `SELECT wardens.avatar,wardens.name FROM wardens WHERE wardens.id = ?`;
...
router.get("/:name?", verifyToken, async function (req, res, next) {
  if (req.params.name) {
    var sql = `SELECT users.id,users.name,users.firstname,users.lastname,users.avatar FROM users WHERE (CONCAT(users.firstname, ' ', users.lastname) LIKE ?) AND users.id IN (SELECT chat.user_id FROM chat WHERE warden_id = ?);`;
    name = "%" + req.params.name + "%";
    await db.query(sql, [name,req.user_id], function (err, result) {
      res.status(201).json({ result: result });
    });
  } else {
    var sql = `SELECT users.id,users.name,users.firstname,users.lastname,users.avatar FROM users WHERE users.id IN (SELECT chat.user_id FROM chat WHERE warden_id = ?);`;
    await db.query(sql,[req.user_id], function (err, result) {
      if (err) throw err;
      res.status(201).json({ result: result });
    });
