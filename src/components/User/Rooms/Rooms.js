...
function Rooms({ match, location }) {
  ...
  useEffect(() => {
    ...
    let getRoomsData = async () => {
      await axios
        .get(`${process.env.React_App_Url}/user/rooms`, {
          headers: {
            token: token,
          },
          params: {
  ...
  const searchRoom = async (name) => {
    setLoading(true);
    await axios
      .get(`${process.env.React_App_Url}/user/rooms/${name}`, {
        headers: {
          token: token,
        },params: {
            category: category
    ...
    <div id="content" className="p-4">
      <div className="card-body">
        ...
        <div className="row d-flex align-items-center justify-content-between mr-1">
          ...
          <div>
            <Link to={`${match.url}/booked-rooms`}>
              <button className="btn btn-outline-dark mr-1">
                <i className="fa fa-briefcase mr-1"></i> Booked Rooms
              </button>
            </Link>
          </div>
        </div>
