...
function Room() {
  ...
  useEffect(() => {
    let getUserData = async () => {
      await axios
        .get(`${process.env.React_App_Url}/user/rooms/show/${id}`, {
          headers: {
            token: token,
          },
        })
  ...
  const book = async (id) => {
    await axios
      .post(
        `${process.env.React_App_Url}/user/rooms/book/`,
        {
          room_id: room.id,
  ...
  const handleSubmit = async (e) => {
    ...
    await axios
      .post(
        `${process.env.React_App_Url}/user/rooms/review`,
        {
          room_id: room.id,
          warden_id: room.warden_id,
          ...
          <div className="col-md-12">
            <div className="card">
              <div className="card-header d-flex justify-content-between align-items-center">
                ...
                <div>
                  <Link
                    to={`/user/rooms/warden-profile/${room.warden_id}`}
                  >
                ...
                {booking ? (
                  ...
                  <button
                    className="btn btn-sm btn-outline-dark"
                    onClick={(e) => book(room.id)}
                  >
                    <i className="fa fa-book mr-1"></i>Book Room
                  </button>
                )}
            </div>
              </div>
